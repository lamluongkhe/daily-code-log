...
def get_user(username):
    ...
    if cached_user:
        return jsonify({
            "source": "redis",
            "data": cached_user.decode()
        })

    # 2. Nếu chưa có trong Redis thì query MySQL
    ...
    # 3. Lưu kết quả vào Redis cache (60s)
    cache.set(f"user:{username}", str(user), ex=60)
